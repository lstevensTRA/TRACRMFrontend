...
export const EditCaseModal: React.FC<EditCaseModalProps> = ({
  ...
  const {
    ...
    defaultValues: {
      title: traCase.title,
      description: traCase.description || '',
      prioritycode: traCase.prioritycode || 2,
      statecode: traCase.statecode,
      statuscode: traCase.statuscode,
      _customerid_value: traCase._customerid_value,
      _customerid_type: traCase._customerid_type
    }
  });

  useEffect(() => {
    if (traCase.customerid) {
      setSelectedCustomer({
        id: traCase.customerid.id,
        name: traCase.customerid.name,
        type: traCase.customerid.type
      });
    }
  }, [traCase]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Edit Case ${traCase.incidentid}`} size="lg">
